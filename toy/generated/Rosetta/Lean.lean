...
def matchPiece : Piece := {
  ...
  grammar := [
    ...
    ("Match.matchScrutinee", ((node "scrNum" (ref "TOKEN.number")).alt ((node "scrStr" (ref "TOKEN.string")).alt ((node "scrParen" (((lit "(").seq (ref "Core.coreTerm")).seq (lit ")"))).alt (node "scrVar" ((ref "TOKEN.ident").seq ((ref "Match.scrArg").star))))))),
    ("Match.scrArg", ((node "scrProj" ((lit ".").seq (ref "TOKEN.ident"))).alt (node "scrProjNum" ((lit ".").seq (ref "TOKEN.number"))))),
    ("Match.matchScrutineeTail", (node "scrTail" ((lit ",").seq (ref "Match.matchScrutinee")))),
