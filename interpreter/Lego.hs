...
module Lego
  ...
  , ParamLang(..), poParamLang, contramapParam
  , constParamLang, diagParamLang
    -- * Natural Transformations
  , ParamNat, applyParamNat, idParamNat, composeParamNat
    -- * Binary Parametric Languages
  , ParamLang2(..), poParamLang2, curryParam, uncurryParam
...
-- | Parametric language: a functor from types to languages
--
-- Mathematical structure:
--   ParamLang lives in the functor category [Type, Lang]
--   Objects: type-indexed language families
--   Morphisms: natural transformations
--
-- Examples:
--   List[A]    : A ↦ Lang with cons : A → List[A] → List[A]
--   Vector[n,A]: (n,A) ↦ Lang with length-indexed operations
--   State[S]   : S ↦ Lang with get : S, put : S → ()
--
-- The key insight: parametric languages enable generic programming
-- while preserving compositional semantics via pointwise pushout.
newtype ParamLang a t = ParamLang { instantiate :: t -> Lang a }

-- | Pushout of parametric languages (pointwise)
--
-- This is the Day convolution / pointwise Kan extension:
--   (F ⊔ G)(t) = F(t) ⊔ G(t)
--
-- Laws (inherited from Lang pushout):
--   Commutativity: F ⊔ G ≅ G ⊔ F
--   Associativity: (F ⊔ G) ⊔ H ≅ F ⊔ (G ⊔ H)
--   Identity: F ⊔ Empty ≅ F (where Empty t = emptyLang)
poParamLang :: ParamLang a t -> ParamLang a t -> ParamLang a t
poParamLang f g = ParamLang $ \t -> poLang (instantiate f t) (instantiate g t)

-- | Map over the type parameter (contravariant in t)
-- This makes ParamLang a presheaf: Type^op → Lang
contramapParam :: (s -> t) -> ParamLang a t -> ParamLang a s
contramapParam h pl = ParamLang $ \s -> instantiate pl (h s)

-- | Constant parametric language (ignores parameter)
constParamLang :: Lang a -> ParamLang a t
constParamLang l = ParamLang (const l)

-- | Diagonal: same language for all types
diagParamLang :: Lang a -> ParamLang a t
diagParamLang = constParamLang

--------------------------------------------------------------------------------
-- Natural Transformations
--------------------------------------------------------------------------------

-- | Natural transformation between parametric languages
-- A family of Lang morphisms indexed by type, commuting with instantiation.
--
-- For F, G : Type → Lang, a natural transformation α : F ⟹ G
-- gives αₜ : F(t) → G(t) for each type t, such that for any f : s → t:
--   αₜ ∘ F(f) = G(f) ∘ αₛ
type ParamNat a t = forall r. (Lang a -> r) -> ParamLang a t -> t -> r

-- | Apply natural transformation
applyParamNat :: ParamNat a t -> (Lang a -> r) -> ParamLang a t -> t -> r
applyParamNat nat = nat

-- | Identity natural transformation
idParamNat :: ParamNat a t
idParamNat f pl t = f (instantiate pl t)

-- | Vertical composition of natural transformations
-- If α : F ⟹ G and β : G ⟹ H, then β ∘ α : F ⟹ H
composeParamNat :: ParamNat a t -> ParamNat a t -> ParamNat a t
composeParamNat beta _alpha f pl t = beta f (ParamLang $ \t' -> 
  let -- Apply alpha to get intermediate result, then use beta's continuation
      intermediate = instantiate pl t'
  in intermediate) t

--------------------------------------------------------------------------------
-- Binary Parametric Languages (Product Category)
--------------------------------------------------------------------------------

-- | Two-parameter language: (t, u) ↦ Lang
-- Lives in [Type × Type, Lang]
--
-- Examples:
--   Map[K,V]     : (K,V) ↦ Lang with insert : K → V → Map[K,V] → Map[K,V]
--   Either[A,B]  : (A,B) ↦ Lang with left : A → Either[A,B], right : B → Either[A,B]
--   Arrow[A,B]   : (A,B) ↦ Lang with arr : (A → B) → Arrow[A,B]
newtype ParamLang2 a t u = ParamLang2 { instantiate2 :: t -> u -> Lang a }

-- | Pointwise pushout for two-parameter languages
poParamLang2 :: ParamLang2 a t u -> ParamLang2 a t u -> ParamLang2 a t u
poParamLang2 f g = ParamLang2 $ \t u -> poLang (instantiate2 f t u) (instantiate2 g t u)

-- | Curry: convert binary to unary parametric language over pairs
curryParam :: ParamLang a (t, u) -> ParamLang2 a t u
curryParam pl = ParamLang2 $ \t u -> instantiate pl (t, u)

-- | Uncurry: convert binary parametric language to unary over pairs
uncurryParam :: ParamLang2 a t u -> ParamLang a (t, u)
uncurryParam pl2 = ParamLang $ \(t, u) -> instantiate2 pl2 t u

-- | Fix first parameter
_fixFirst :: ParamLang2 a t u -> t -> ParamLang a u
_fixFirst pl2 t = ParamLang $ \u -> instantiate2 pl2 t u

-- | Fix second parameter  
_fixSecond :: ParamLang2 a t u -> u -> ParamLang a t
_fixSecond pl2 u = ParamLang $ \t -> instantiate2 pl2 t u

-- | Flip parameter order
_flipParam :: ParamLang2 a t u -> ParamLang2 a u t
_flipParam pl2 = ParamLang2 $ \u t -> instantiate2 pl2 t u
